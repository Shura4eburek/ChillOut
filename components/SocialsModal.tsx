...
import { X, Send, Gamepad, Instagram, Twitch, ExternalLink } from 'lucide-react';
...
export const SocialsModal: React.FC<SocialsModalProps> = ({ isOpen, onClose }) => {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b-2 border-gray-800 bg-[#252525]">
          <h2 className="text-xl font-bold text-white drop-shadow-md uppercase tracking-wider">Community</h2>
          <button 
            onClick={handleClose}
        ...
        <div className="p-5 overflow-hidden">
          <SocialItem 
            index={0}
            title="Telegram"
            subtext="@ChillOutProject"
            icon={<Send size={20} className="-ml-0.5 mt-0.5" />} // Offset for visual centering
          ...
          <SocialItem 
            index={2}
            title="Instagram"
            subtext="Daily server showcases"
          ...
          <SocialItem 
            index={3}
            title="Twitch"
            subtext="Live events every Friday"
            icon={<Twitch size={24} />}
